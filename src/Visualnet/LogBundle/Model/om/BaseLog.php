...
/**
 * Base class that represents a row from the 'visual_log' table.
 *
 * 
 *
...
abstract class BaseLog extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'Visualnet\\LogBundle\\Model\\LogPeer';
	...
	public function setUserId($v)
	{
		...
		if ($this->user_id !== $v) {
			$this->user_id = $v;
			$this->modifiedColumns[] = LogPeer::USER_ID;
		}
	...
	public function setType($v)
	{
		...
		if ($this->type !== $v) {
			$this->type = $v;
			$this->modifiedColumns[] = LogPeer::TYPE;
		}
	...
	public function setContent($v)
	{
		...
		if ($this->content !== $v) {
			$this->content = $v;
			$this->modifiedColumns[] = LogPeer::CONTENT;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 8; // 8 = LogPeer::NUM_HYDRATE_COLUMNS.
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($isInsert) {
				...
				if (!$this->isColumnModified(LogPeer::UPDATED_AT)) {
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(LogPeer::USER_ID)) {
			$modifiedColumns[':p' . $index++]  = '`USER_ID`';
		}
		if ($this->isColumnModified(LogPeer::TYPE)) {
			$modifiedColumns[':p' . $index++]  = '`TYPE`';
		}
		if ($this->isColumnModified(LogPeer::MESSAGE)) {
			$modifiedColumns[':p' . $index++]  = '`MESSAGE`';
		}
		if ($this->isColumnModified(LogPeer::IP)) {
			$modifiedColumns[':p' . $index++]  = '`IP`';
		}
		if ($this->isColumnModified(LogPeer::CONTENT)) {
		...
		$sql = sprintf(
			'INSERT INTO `visual_log` (%s) VALUES (%s)',
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setMesssage($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(LogPeer::USER_ID)) $criteria->add(LogPeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(LogPeer::TYPE)) $criteria->add(LogPeer::TYPE, $this->type);
		...
		if ($this->isColumnModified(LogPeer::CONTENT)) $criteria->add(LogPeer::CONTENT, $this->content);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setMesssage($this->getMesssage());
